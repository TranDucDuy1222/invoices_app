...
class LsHoaDonView(tk.Frame):
    ...
    def create_widgets(self):
        ...
        # THAY ĐỔI: Đặt chiều rộng tối thiểu cho cột danh sách (cột 0)
        # và cho phép cột chi tiết (cột 1) co giãn.
        self.grid_columnconfigure(0, weight=0, minsize=500) # Cột 0 sẽ có chiều rộng tối thiểu 500px
        self.grid_columnconfigure(1, weight=1) # Cột 1 sẽ chiếm hết phần không gian còn lại
        ...
        self.tree_detail.heading("phi_vc", text="Phí vận chuyển")
        self.tree_detail.heading("thanh_tien", text="Thành tiền")
        self.tree_detail.heading("noi_giao", text="Nơi giao")

        # THAY ĐỔI: Sử dụng minwidth thay vì width cố định để cột có thể mở rộng
        # và kích hoạt thanh cuộn ngang.
        self.tree_detail.column("ngay_mua", minwidth=80, width=80, anchor="center")
        self.tree_detail.column("mat_hang", minwidth=120, width=150, anchor="center")
        self.tree_detail.column("noi_lay", minwidth=100, width=120, anchor="center")
        self.tree_detail.column("so_xe", minwidth=80, width=90, anchor="center")
        self.tree_detail.column("don_vi", minwidth=70, width=70, anchor="center")
        self.tree_detail.column("so_luong", minwidth=80, width=80, anchor="center")
        self.tree_detail.column("gia_tai_bai", minwidth=100, width=100, anchor="center")
        self.tree_detail.column("phi_vc", minwidth=100, width=100, anchor="center")
        self.tree_detail.column("thanh_tien", minwidth=110, width=110, anchor="center")
        self.tree_detail.column("noi_giao", minwidth=250, width=320) # Tăng chiều rộng tối thiểu cho cột này
        
        # --- THÊM THANH CUỘN NGANG VÀ DỌC ---
        # Thanh cuộn dọc
        detail_scrollbar_y = ttk.Scrollbar(detail_tree_container, orient="vertical", command=self.tree_detail.yview)
        self.tree_detail.configure(yscrollcommand=detail_scrollbar_y.set)

        # Thanh cuộn ngang
        detail_scrollbar_x = ttk.Scrollbar(detail_tree_container, orient="horizontal", command=self.tree_detail.xview)
        self.tree_detail.configure(xscrollcommand=detail_scrollbar_x.set)

        # Sắp xếp các widget vào grid
        ...
        # THÊM: Gán sự kiện cuộn chuột cho thanh cuộn ngang
        # Gán sự kiện cuộn dọc, cuộn ngang (với phím Shift hoặc Control)
        self.tree_detail.bind("<MouseWheel>", self._on_vertical_scroll)
        self.tree_detail.bind("<Shift-MouseWheel>", self._on_horizontal_scroll)
        self.tree_detail.bind("<Control-MouseWheel>", self._on_horizontal_scroll) # Thêm cho trackpad
    ...
    def _on_vertical_scroll(self, event):
        """Xử lý sự kiện cuộn dọc."""
        self.tree_detail.yview_scroll(int(-1 * (event.delta / 120)), "units")

    def _on_horizontal_scroll(self, event):
        """Xử lý sự kiện cuộn ngang (khi giữ Shift hoặc Control)."""
        self.tree_detail.xview_scroll(int(-1 * (event.delta / 120)), "units")
        ...
        if hoa_don_info:
            ...
            if self.controller:
                ...
                for detail in details:
                    ...
                    thanh_tien_formatted = f"{int(float(detail[3])):,}".replace(",", ".") if detail[3] else "0"
                    don_gia_formatted = f"{int(float(detail[8])):,}".replace(",", ".") if detail[8] else "0"
                    phi_vc_formatted = f"{int(float(detail[9])):,}".replace(",", ".") if detail[9] else "0"
